...
static void lcd_status_screen();
  ...
  static void lcd_ver_menu();
  static void lcd_verr();
...
static void lcd_ver_menu() {
  ...
  //if (LCD_CLICKED) lcd_goto_menu(lcd_status_screen); 
}
/**
 *
 * "Tune" submenu
 ...
 */
  ...
  MENU_MULTIPLIER_ITEM_EDIT(int3, MSG_LASER, &laser_driver_ttl, 0, 255);
  if(laser_driver_ttl == 0){
  ...
  MENU_ITEM(submenu, MSG_ABOUT, lcd_ver_menu);
  END_MENU();
}
...
#if ENABLED(DOGLCD)
static void lcd_verr() {
  u8g.drawStr(5,13,"\x4d\x61\x72\x6c\x69\x6e\x20\x76\x2e\x31\x2e\x31\x2e\x30\x2d\x52\x43\x32");
  u8g.drawStr(5,23,"\x76\x65\x72\x2e\x30\x31\x33\x20\x28\x31\x30\x35\x31\x32\x30\x36\x29");
  u8g.drawStr(5,33,"\x3e\x4c\x41\x53\x45\x52");
  u8g.drawStr(5,44,"\x3e\xa3\xa4\xa5\xa6");
  u8g.drawStr(70,60,"\x58\x4a\x2e\x48\x73\x75\x5f");
}
#endif
/**
 *
...
#if DISABLED(DOGLCD)
static void lcd_verr() {
  lcd_print("Marlin v.1.1.0-RC2");
  lcd_print("ver.013 (1051206)");
  lcd_print(">Laser");
  lcd_print("XJ.Hsu _");
}
#endif
