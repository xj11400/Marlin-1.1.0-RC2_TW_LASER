...
#define FONT_SPECIAL_NAME Marlin_symbols
  ...
  #if ENABLED(DISPLAY_CHARSET_tc_TW)
    #include "dogm_font_data_tc_TW.h"
    #define FONT_MENU_NAME tc_TW
...
//laser
  ...
  lcd_setFont(FONT_STATUSMENU);
  u8g.setPrintPos(laserpos,7);
      int lpwper = laser_driver_ttl;
    if (lpwper) {
      lcd_print(itostr3(lpwper));
    }
    else
    {
      lcd_printPGM(PSTR("---"));
    }
  //u8g.drawStr(40,17,"Laser");
  
  if (lpwper==0)
  
  u8g.drawBitmapP(laserpos+3,8, LASER_BYTEWIDTH, LASER_HEIGHT, laseroff_bmp);
  else
  u8g.drawBitmapP(laserpos+3,8,LASER_BYTEWIDTH,LASER_HEIGHT, (blink % 2) && lpwper ? laseron0_bmp : laseron1_bmp);
  

  
  u8g.setPrintPos(laserpos,28);
    int lper = ((laser_driver_ttl + 1) * 100) / 256;
    if (laser_driver_ttl) {
      lcd_print(itostr3(lper));
      lcd_print('%');
    }
    else
    {
